...
export class LastTimeServer {
  ...
  generateHistory(
    ...
    let sql =
      "SELECT lth_id, max(lth_num_sequential) as lth_num_sequential from vilasttimehistory";
    ...
    return connection.runSql(sql).then(sqlData => {
      ...
      modelList.forEach((m: LastTime) => {
        ...
        } = sqlData.rows.find((r: any) => r.lth_id === m.lst_id);

        itemList.push(
          new LastTimeHistory({
            lth_id: m.lst_id,
            lth_num_sequential: historyItem
              ? historyItem.lth_num_sequential + 1
              : 1,
            lth_name: m.lst_name,
  ...
  async _initializeDataFromBackup() {
    ...
    const LastTimeList: LastTime[] = await backupCon
      .runSql("select * from vilasttime")
      .then(data => {
        return data.rows.map((e: any) => new LastTime(e));
      });
    const sqlMotor: MoSQL = new MoSQL(new LastTime());
    const response: any[] = await Promise.all(
      defaultCon.runSqlArray(LastTimeList.map(e => sqlMotor.toInsertSQL(e)))
    );
    return response;
  }
  ...
  async _backupLastTimeData() {
    ...
    const LastTimeDefaultList: LastTime[] = await defaultCon
      .runSql("select * from lasttime")
      .then(data => {
        return data.rows.map((e: any) => new LastTime(e));
      });
    const LastTimeBackupList: LastTime[] = await backupCon
      .runSql("select * from lasttime")
      .then(data => {
        return data.rows.map((e: any) => new LastTime(e));
      });
    ...
    LastTimeDefaultList.forEach(defItem => {
      const existentItem: LastTime = LastTimeBackupList.find(
        item => item.lst_id === defItem.lst_id
      );
      if (existentItem) {
        // update if different
    ...
    const LastTimeHistoryBackupList: LastTimeHistory[] = await backupCon
      .runSql("select * from lasttimehistory")
      .then(data => {
        return data.rows.map((e: any) => new LastTimeHistory(e));
      });
    const LastTimeHistoryDefaultList: LastTimeHistory[] = await defaultCon
      .runSql("select * from lasttimehistory")
      .then(data => {
        return data.rows.map((e: any) => new LastTimeHistory(e));
      })
      .then((defaultList: LastTimeHistory[]) => {
        ...
        defaultList.forEach(item => {
          const foundInNewList: LastTimeHistory = max(
            newList.filter(b => b.lth_id === item.lth_id)
          );
          if (foundInNewList) {
            item.lth_num_sequential = foundInNewList.lth_num_sequential + 1;
          } else {
            const foundInDefault: LastTimeHistory = max(
              LastTimeHistoryBackupList.filter(b => b.lth_id === item.lth_id)
            );
            if (foundInDefault) {
    ...
    const historyResponse: any[] = await Promise.all(
      backupCon.runSqlArray(
        LastTimeHistoryDefaultList.map(item => sqlMotor.toInsertSQL(item))
      )
    );
  ...
  async _cleanUpData() {
    const defaultCon: iConnection = ConnectionService.getConnection();
    const response: any = await defaultCon.runSql(
      "delete from lasttimehistory"
    );
    return response;
  }
}
