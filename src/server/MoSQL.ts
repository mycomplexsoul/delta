...
export class MoSQL {
      ...
      .forEach((f) => {
        ...
        sql = this.concatSemicolon(
          ...
          this.formatValueForSQL(
            f.dbType,
            m[f.dbName] !== null && m[f.dbName] !== undefined
              ? m[f.dbName]
              : null
          )
        );
  ...
  toChangesObject(model: iEntity, changes: iEntity | Array<any>): any[] {
    ...
    const handlePossibleChange = (
      ...
      if (areDifferent) {
        // change value diff from current
  ...
  toUpdateSQL(changes: iEntity | Array<any>, model?: iEntity): string {
    ...
    changesArray.forEach((change: any) => {
      ...
      sqlChanges = this.concatSemicolon(
        sqlChanges,
        this.formatValueForSQL(change.dbType, change.value, change.dbName)
      );
      //}
    });
  ...
  toSelectPKPlaceholderSQL = (model?: iEntity): string => {
    ...
    this.getPK(m).forEach((field, index: number) => {
      sql = this.concatAnd(
        sql,
        this.formatValueForSQL(field.dbType, `{${field.dbName}}`, field.dbName)
      ); // This will not work if dbType is Date, if a PK is Date fix it
    });
