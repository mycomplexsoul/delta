...
const sendReceiptEmail = async (
  ...
  const prefix = `${DateUtils.getMonthNameSpanish(month)
    .substring(0, 3)
    .toUpperCase()}${year - 2000}`;
  ...
  const provisionList: CarteraProvision[] = (await getProvisions())
    .filter((p) => (unit ? p.cpr_id_unit == unit : true) && p.cpr_folio)
    .filter((p) => p.cpr_folio.includes(prefix))
    .sort((a, b) =>
      // order by folio
      parseInt(a.cpr_folio.substr(a.cpr_folio.indexOf("-") + 1, 3), 10) >
      parseInt(b.cpr_folio.substr(b.cpr_folio.indexOf("-") + 1, 3), 10)
        ? 1
        : -1
    );
    ...
    provisionList.map(
      (p, index) =>
        new Promise((resolve, reject) => {
          ...
          const subject = `FFJ78 Recibo de ${
            mapCodeWithDescription[p.cpr_type]
          } ${monthName} ${p.cpr_year}`;
          const filename = `${p.cpr_folio} Recibo de pago ${p.cpr_id_unit} - ${
            mapCodeWithDescription[p.cpr_type]
          } ${monthName} ${p.cpr_year}.pdf`;
          const filepath = `${configModule.getConfigValue(
            "receipts-path"
          )}${filename}`;
          ...
          if (file_exists) {
            try {
              // wait 4s to send email, to ensure we don't exceed API limits
              setTimeout(() => {
                EmailModule.sendHTMLEmail({
                  subject,
                  html: body,
                  to,
                  attachments: [
  ...
  console.log("--allPromises status", allPromises);
