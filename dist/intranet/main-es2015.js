...
let TasksComponent = class TasksComponent {
    ...
    calculateIndicators() {
        ...
        let addIndicator = (name, values, showIndicator = false, formatMethod, completedCriteria) => {
            if (!showIndicator) {
                return;
            }
            let formattedValues = [];
        ...
        addIndicator("Open Count EOD", calculateForAllDays(days, this.services.taskIndicator.totalTaskCountUntil), this.options["optShowIndicatorOpenCountEOD"], null, (prev, curr) => ({
            isCompleted: prev >= curr,
            percentageCompleted: prev !== 0 ? Math.round((curr * 100) / prev) / 100 : 0
        }));
        // added ETA
        addIndicator("Added ETA", calculateForAllDays(days, this.services.taskIndicator.addedETA), this.options["optShowIndicatorAddedETA"], (v) => this.formatTime(v * 60), (prev, curr) => ({
            isCompleted: prev >= curr,
            percentageCompleted: prev !== 0 ? Math.round((curr * 100) / prev) / 100 : 0
        }));
        // added count
        addIndicator("Added Count", calculateForAllDays(days, this.services.taskIndicator.addedTaskCount), this.options["optShowIndicatorAddedCount"], null, (prev, curr) => ({
            isCompleted: prev >= curr,
            percentageCompleted: prev !== 0 ? Math.round((curr * 100) / prev) / 100 : 0
        }));
        // closed ETA
        addIndicator("Closed ETA", calculateForAllDays(days, this.services.taskIndicator.closedETA), this.options["optShowIndicatorClosedETA"], (v) => this.formatTime(v * 60));
        // spent
        addIndicator("Closed Spent", calculateForAllDays(days, (d1, d2) => this.services.taskIndicator.calculateTotalTimeSpent(d1, d2)
            .totalTimeSpentTodayOnClosedTasks), this.options["optShowIndicatorClosedSpent"], (v) => this.formatTime(v));
        // closed count
        ...
        // Spent on Open Tasks
        addIndicator("Open Spent", calculateForAllDays(days, (d1, d2) => this.services.taskIndicator.calculateTotalTimeSpent(d1, d2)
            .totalTimeSpentTodayOnOpenTasks), this.options["optShowIndicatorOpenSpent"], (v) => this.formatTime(v));
        // karma
    }
    asNextToDo(t, add) {
