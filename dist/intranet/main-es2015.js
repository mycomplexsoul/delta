...
let TasksComponent = class TasksComponent {
    ...
    calculateIndicators() {
        ...
        let addIndicator = (name, values, formatMethod, completedCriteria) => {
            let formattedValues = [];
            if (formatMethod) {
        ...
        addIndicator("Open Count EOD", calculateForAllDays(days, this.services.taskIndicator.totalTaskCountUntil), null, (prev, curr) => ({
        ...
        // Open ETA
        addIndicator("Open ETA", calculateForAllDays(days, this.services.taskIndicator.openETA), (v) => this.formatTime(v * 60), (prev, curr) => ({
            isCompleted: prev >= curr,
            percentageCompleted: prev !== 0 ? Math.round((curr * 100) / prev) / 100 : 0
        }));
        // Spent on Open Tasks
        addIndicator("Open Spent", calculateForAllDays(days, (d1, d2) => this.services.taskIndicator.calculateTotalTimeSpent(d1, d2)
            .totalTimeSpentTodayOnOpenTasks), (v) => this.formatTime(v));
        // karma
    }
    asNextToDo(t, add) {
...
let TasksCore = class TasksCore {
    constructor(http, sync, dateUtils) {
        this.http = http;
        this.sync = sync;
        this.dateUtils = dateUtils;
    ...
    getAllForUser(user) {
        return this.getAll().then((list) => {
            return list.filter((x) => x.tsk_id_user_asigned === user);
        });
    }
    /** BEGIN API methods */
    /**
